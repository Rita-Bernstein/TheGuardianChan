...
public class DoubleSlimeParticle extends com.megacrit.cardcrawl.vfx.AbstractGameEffect {
    ...
    public DoubleSlimeParticle(AbstractPlayer p) {
        if(SlimeOriginalAnimation){
            ...
            this.atlas = new TextureAtlas(Gdx.files.internal("TheGuardianChan/monsters/TheSlimeBossWaifu/char/Slime_acid_char.atlas"));
            SkeletonJson json = new SkeletonJson(this.atlas);



            json.setScale(Settings.scale / scale);
            SkeletonData skeletonData = json.readSkeletonData(Gdx.files.internal("TheGuardianChan/monsters/TheSlimeBossWaifu/char/Slime_acid_char.json"));
            this.skeleton = new Skeleton(skeletonData);
            this.skeleton.setColor(Color.WHITE);
            this.stateData = new AnimationStateData(skeletonData);
            this.state = new AnimationState(this.stateData);
